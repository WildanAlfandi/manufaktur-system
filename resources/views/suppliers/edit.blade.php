                    ...
                    <form method="POST" action="{{ route('suppliers.update', $supplier) }}">
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="address" id="address" rows="3" required
                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('address', $supplier->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700">
                                        Total Transaksi: <span class="font-bold">{{ $supplier->transactions()->count() }}</span>
                                    </p>
                                    <p class="text-sm text-blue-700">
                                        Terdaftar Sejak: <span class="font-bold">{{ $supplier->created_at->format('d/m/Y') }}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex items-center justify-end gap-4">
                            <a href="{{ route('suppliers.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded">
                                Batal
                            </a>
